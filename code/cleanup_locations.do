...
foreach v in dpto prov dist {
	...
	*replace `v' = subinstr(`v', char(195) + char(154), "U", .)
	...
	replace `v' = "" if inlist(`v', "NULL", "NO REGISTRADO", "EN INVESTIGACION")
}

replace prov = "CALLAO" if prov == "PROV. CONST. DEL CALLAO"
replace dpto = "CALLAO" if prov == "CALLAO" & mi(dpto)
replace dpto = "CALLAO" if dpto == "LIMA" & prov == "CALLAO" & inlist(dist, "CALLAO", "VENTANILLA", "CARMEN DE LA LEGUA REYNOSO")
...
replace dist = "ANDRES AVELINO CACERES" if dpto == "AYACUCHO" & prov == "HUAMANGA" & strpos(dist, "AVELINO")
...
replace dist = "CORONEL GREGORIO ALBARRACIN LANCHIPA" if dpto == "TACNA" & prov == "TACNA" & strpos(dist, "CORONEL GREGORIO")
