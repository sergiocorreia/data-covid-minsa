...
foreach female in 0 1 {
	if (`female' == 0) loc genero "hombres"
	if (`female' == 1) loc genero "mujeres"
foreach group in 1 2 3 4 5 {
	preserve
	keep if (group==`group') & (is_female == `female')
	...
	tw	///
		...
		`cmd' ///
		, ///
		title("Fallecimientos `rango' años en Lima Metrop. (`genero')") yscale(range(0 0)) ///
		xtitle("Semana") ///
		ytitle("Casos", margin(right) ) ///
		xlabel(0(4)52) ///
		ylabel(`r(ticks)', format(%6.0fc) angle(horizontal)) ///
		scheme(s2color) ///
	...
	graph export "../figures/excess_mortality_lima_metro_`group'_`female'.png", replace width(2000)
	graph export "../figures/excess_mortality_lima_metro_`group'_`female'.pdf", replace
	restore
}
}
exit
